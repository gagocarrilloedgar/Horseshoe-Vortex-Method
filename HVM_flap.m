...
delta25=input('Quarter chord swep angle (�): ');
delta25=deg2rad(delta25);

%TWIST
disp('Aerodynamic twist ' );
alpha_cr=input('Root airfoil epsilon (º): ');
alpha_cr=deg2rad(alpha_cr);
alpha_ct=input('Tip airfoil epsilon (º): ');
...
alphal0t = input('Zerolift tip alpha (�): ');
alphal0t=deg2rad(alphal0t);
alphal0r = input('Zerolift root alpha(�): ');
            ...
            if (xh > 1)
                ...
                inputposition=0;
                    while (inputposition ==0)
                        %Flap position
                        y1= input('Introduce the starting flap position 0-0.5: ');
                        y2 =input('Introduce the ending flap position 0-0.5: ');
                
                        if(y1>0.5 || y2>0.5 ) 
                            disp(' Introduce a valid flap position ');
                            fprintf('\n');
                        ...
                        %Incremento que produce el flap:   
                        if (flap1 ==1)
                            alphal0_flap=-((1-(deg2rad(thetah)/pi)+(sin(thetah)/pi)))*rad2deg(eta);
                            Cm0flap=-0.5*sin(thetah)*(1-cos(thetah))*rad2deg(eta);
                        end
