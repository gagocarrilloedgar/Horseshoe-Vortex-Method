...
yflapinicial=input('Initial flap y-position (y/b): ');
yflapfinal=input('Final flap y-position (y/b): ');
incrementalphal0=input('Increment alphal0 due to flap: ');

    
disp('Geometric angle of attack');
alpha = input('alpha (º): ');
alpha=deg2rad(alpha); 
beta=deg2rad(0);
...
contador=1;
while (contador<=N)
    if ((yb(contador))>=(yflapinicial)) && ((yc(contador))<=yflapfinal)
    alphal02(contador)=alphal0(contador)-incrementalphal0;
    contador=contador+1;
    else
    contador=contador+1;
    end
