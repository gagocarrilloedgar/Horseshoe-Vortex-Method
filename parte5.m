...
eta = deg2rad(eta);
thetah = acos(1-(2*xh));
alphal0_flap=-((1-(deg2rad(thetah)/pi)+(sin(thetah)/pi)))*eta;
alphal0_flap=radtodeg(alphal0_flap;
Cm0flap=-0.5*sin(thetah)*(1-cos(thetah))*rad2deg(eta);

%Factores con flap
alphal0t=-2.057+alphal0_flap;
alphal0t=deg2rad(alphal0t);%del HMW1
alphal0r=-2.057+alphal0_flap;
