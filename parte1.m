...
aspect_ratio=zeros(1,10);
aspect_ratio1=[2 3 4 5 6 7 200];
CL_alpha_teorico=[2.5 3.1 3.6 4 4.2 4.4 6.25];
CL=zeros(1,10);
CL_alpha=zeros(1,10);
...
aspect_ratio(k)=k+5;
S=1/aspect_ratio(k);
taper_ratio=1; %(rectangular )
cr=(2/aspect_ratio(k))/(1+taper_ratio);
ct=cr*taper_ratio;
...
for i=1:N
    ...
    sumaCL=CL;
    CMle=-(2/(S*cmg))*Circulacion(i)*(xvhm(i)+abs(yvhm(i))*tandeltaLE)*(1/N)+sumaCMle;
    sumaCMle=CMle;
    Cly(i)=(2/s(i))*Circulacion(i)*(1/N);
    % como es un perfil simetrico alphal0 es 0 y cogemos alpha como 4 grados.
...
plot(aspect_ratio,CL_alpha,'c*','DisplayName','HVM');
title('$CL_{\alpha}$ as fucntion of A ','interpreter','latex');
xlabel('Aspect Ratio A','interpreter', 'latex' );
ylabel('$CL_{\alpha}$','interpreter','latex');

hold on;
plot(aspect_ratio1,CL_alpha_teorico,'o','DisplayName','Katz and Plotkin');
grid on;
legend show;
